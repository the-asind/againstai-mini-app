...
class SocketServiceImpl {
    ...
    public setCurrentLobbyCode(code: string | null) {
    ...
    public updateSettings(code: string, settings: Partial<LobbySettings>) {
        this.socket?.emit('update_settings', { code, settings, playerId: this.currentPlayer?.id });
    }

    public updatePlayer(code: string, updates: Partial<Player>) {
        this.socket?.emit('update_player', { code, updates, playerId: this.currentPlayer?.id });
    ...
    public getAggregateNavyUsage(code: string) {
        this.socket?.emit('get_aggregate_navy_usage', { code, playerId: this.currentPlayer?.id });
    }

    public startGame(code: string) {
        this.socket?.emit('start_game', { code, playerId: this.currentPlayer?.id });
    }

    public submitAction(code: string, action: string) {
        this.socket?.emit('submit_action', { code, action, playerId: this.currentPlayer?.id });
    }

    public revealResults(code: string) {
        this.socket?.emit('reveal_results', { code, playerId: this.currentPlayer?.id });
    }

    public resetGame(code: string) {
        this.socket?.emit('reset_game', { code, playerId: this.currentPlayer?.id });
    }

    public nextRound(code: string) {
        this.socket?.emit('next_round', { code, playerId: this.currentPlayer?.id });
    }
