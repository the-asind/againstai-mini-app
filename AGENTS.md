...
| **`server/services/lobbyService.ts`** | **Менеджер состояний Lobbies.** Управляет фазами (`HOME` -> `LOBBY_WAITING` -> `SCENARIO_GENERATION` -> `PLAYER_INPUT` -> `JUDGING` -> `RESULTS`).<br> • Обработка таймеров фазы ввода хода.<br> • Синхронизация генерации (Текст -> Картинка -> Озвучка).<br> • Приватная рассылка индивидуальных ИИ-секретов (`emitSecrets`).<br> • **Multi-Round Logic (`nextRound`)**: Накопление стейта инвентаря и эффектов из раунда в раунд.<br> • **Authoritative Probability Wheel**: Серверная генерация 20 сегментов колеса с использованием алгоритма «Шага» (Stride 7) для равномерного распределения BOSS/SPECIAL сегментов. Полная синхронизация визуального результата с бэкендом. |
| **`server/services/geminiService.ts`** | **Мозг Гейм-Мастера.** <br> • Обертка `extractJSON`: устойчивый парсер, вырезающий JSON-данные из грязного текста LLM (если ИИ примешивает "размышления").<br> • `generateScenario()`: Создает лор, локацию, и теперь **сразу же генерирует ИИ-секреты** (Асимметричный геймплей) для игроков одним LLM вызовом, чтобы сохранить контекст и сэкономить токены. Обрабатывает спец-раунды (`PANIC`, `BODY_SWAP`).<br> • `judgeRound()`: Оценивает действия, решает кто выжил с учетом секретных вводных игроков и инвентаря. |
...
| **`server/utils/logger.ts`** | **Централизованный логгер.** Заменяет встроенный `console`. Форматирует и сохраняет логи в файлы (`error.log`, `combined.log`, `ai-responses.log`). |
...
2. **Многофазная загрузка:** Генерация контента (сценарий + картинка + звук) занимает долгое время (иногда 10-15 секунд). Поэтому добавлены состояния `SCENARIO_GENERATION` с фазами `WHEEL`, `VOTING`, `VOTING_RESULTS`. Любые тяжелые вызовы (или цепочки ИИ-вызовов) должны покрываться UI-событиями. **Колесо вероятностей (WheelConfig) всегда генерируется на сервере** для исключения рассинхрона.
...
6. **Логирование бэкенда:** Запрещено использовать `console.log`. Обязательно импортируй `import logger from './utils/logger'` и используй уровни `logger.info`, `logger.warn`, `logger.error`. Большие простыни текстов от ИИ направляй через функцию `logAiResult`.
7. **Тестирование (TDD):** Бэкенд тесты (`server/*.test.ts`) работают на `bun:test`. Фронтенд изолирован на Vitest (`npm run test:ui`). Для новых UI-вещей создавай `.test.tsx` файлы.
