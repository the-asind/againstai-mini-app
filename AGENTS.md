...
| **`server/services/geminiService.ts`** | **Мозг Гейм-Мастера.** <br> • `generateScenario()`: Создает лор и локацию.<br> • `generateSecrets()`: Асимметричный геймплей. Выдает игрокам уникальные цели (твисты, предатели) перед началом раунда.<br> • `judgeRound()`: Оценивает действия, решает кто выжил с учетом секретных вводных игроков.<br> • `checkInjection()`: Базовая модерация текста игроков. |
...
5. **Влияние UI на UX:** Использовать анимации (Tailwind `animate-pulse`, `animate-bounce` и кастомные `animate-shake`), плавные переходы (`transition-all opacity-0 -> 100`) и встроенные функции TWA (`HapticFeedback.impactOccurred`, `notificationOccurred`). Мы делаем *сочный* Mini App, а не сухую форму ввода.
